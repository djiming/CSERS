...
function init() {
    with (document.creator.province) {
        length = where.length;
        for (var k = 0; k < where.length; k++) { options[k].text = where[k].loca; options[k].value = where[k].loca; }
        //options[selectedIndex].text = where[0].loca; options[selectedIndex].value = where[0].loca;
    }
    with (document.creator.city) {
        loca3 = (where[0].locacity).split("|");
        length = loca3.length;
        for (var l = 0; l < length; l++) { options[l].text = loca3[l]; options[l].value = loca3[l]; }
